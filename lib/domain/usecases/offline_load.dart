...
class SaveQuestionsInDbUsecase extends Usecase<void> {
        ...
        .map(
          (rawQuestion) => QuestionDto(
            ...
            type: QuestionType.values.firstWhere(
                (type) => type.name == rawQuestion['type'],
                orElse: () => QuestionType.choice),
            category: QuestionCategory.values.firstWhere(
                (type) => type.name == rawQuestion['category'],
                orElse: () => QuestionCategory.history),
          ),
        )
        .toList();
    final answers =
        rawQuestions.fold<List<AnswerDto>>([], (previousValue, rawQuestion) {
      final entries = (rawQuestion['answers'] as List)
          .map((a) => Map<String, dynamic>.from(a))
          .map((rawAnswer) {
        return AnswerDto(
          value: rawAnswer['response'] is bool
              ? (rawAnswer['response'] == true ? 'Vrai' : 'Faux')
              : rawAnswer['response'],
          type: AnswerType.values.firstWhere(
              (element) => element.name == rawAnswer['type'],
  ...
  Future<List<Map<String, dynamic>>> _loadQuestionsFromAsset() async {
    ...
    return (json.decode(jsonString)['questions'] as List)
        .map<Map<String, dynamic>>((data) => Map<String, dynamic>.from(data))
        .toList();
  }
}
